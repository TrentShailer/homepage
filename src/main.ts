...
for (let i = 0; i < cards.length; i++) {
    ...
    bg.setAttribute("style", `background: ${colors[color_index][0]}`);

    const icon = card.querySelector<HTMLElement>(".card-icon")!;
    icon.setAttribute("style", `color:${colors[color_index][0]};opacity:0.20;`);

    card.addEventListener("mouseenter", () => {
        icon.setAttribute(
            "style",
            `color:${colors[color_index][1]};opacity:0.33;`
        );
    });
    card.addEventListener("mouseleave", () => {
        icon.setAttribute(
            "style",
            `color:${colors[color_index][0]};opacity:0.20;`
        );
    });
}
